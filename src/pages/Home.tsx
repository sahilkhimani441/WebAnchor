  ...
  return (
    <main className="pt-32 pb-20">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        "@type": "LocalBusiness",
        "name": "WebAnchor Web Design",
        "image": "https://images.unsplash.com/photo-1607472586893-edb57cb31328?auto=format&fit=crop&q=80&w=800",
        "url": "https://webanchor.co.uk",
        "telephone": "+447825182199",
        "address": {
          "@type": "PostalAddress",
          "streetAddress": "North End",
      ...
      <section id="case-studies" className="py-24 bg-white border-t border-[#EAE8E1]/50">
         <div className="max-w-6xl mx-auto px-6">
            <div className="text-center mb-16 flex flex-col items-center">
               <span className="px-3 py-1 bg-[#F8F7F4] border border-[#EAE8E1] text-[#8B4A46] rounded-full text-[10px] font-bold uppercase tracking-wider mb-6 inline-block">Case Studies</span>
               <h2 className="text-3xl md:text-5xl font-medium tracking-tight text-slate-900 max-w-2xl" style={{ fontFamily: "serif" }}>
                  High-converting templates proven in Portsmouth.
               </h2>
               <p className="mt-6 text-lg text-slate-600 max-w-xl font-light">
                  We use proven layouts that turn local visitors into calls. Check out these recent success stories using our specialized trade templates. 
               </p>
            </div>
            ...
            <div className="grid md:grid-cols-3 gap-8">
               {[
                  {
                     img: "https://images.unsplash.com/photo-1607472586893-edb57cb31328?auto=format&fit=crop&q=80&w=800",
                     title: "Plumbing Service Template",
                     subtitle: "High-Conversion Layout",
                     tags: ["Plumbing", "Lead Generation"],
                     desc: "A mobile-first template designed specifically for plumbers. It features strong 'Call Now' action buttons, trust signals, and a clean layout to ensure emergency call-outs are booked instantly.",
                  },
                  {
                     img: "https://images.unsplash.com/photo-1621905252507-b35492cc74b4?auto=format&fit=crop&q=80&w=800",
                     title: "Electrical Experts Template",
                     subtitle: "Trust-Building Design",
                     tags: ["Electrician", "Services"],
                     desc: "Built to showcase certifications and reviews right at the top. This template includes a seamless quote form that is proven to generate qualified residential and commercial leads.",
                  },
                  {
                     img: "https://images.unsplash.com/photo-1588196749597-9e5c9b68832a?auto=format&fit=crop&q=80&w=800",
                     title: "Landscaping & Gardening Template",
                     subtitle: "Visual Portfolio Focus",
                     tags: ["Gardening", "Landscaping"],
                     desc: "An outdoor service template providing large image galleries to showcase 'before & after' garden transformations, coupled with integrated WhatsApp chat to book seasonal schedules quickly.",
                  }
               ].map((project, i) => (
                  <motion.div 
                     key={i}
                     initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: i * 0.15 }}
                     className="group flex flex-col bg-[#F8F7F4] rounded-[2rem] border border-[#EAE8E1] overflow-hidden hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500"
                  >
                     ...
                     <div className="p-8 flex-1 flex flex-col">
                        <div className="flex flex-wrap gap-2 mb-4">
                           {project.tags.map(tag => (
                              <span key={tag} className="px-2 py-1 bg-white border border-[#EAE8E1] text-[#8B4A46] text-[10px] font-bold uppercase tracking-wider rounded-md">
                                 {tag}
                              </span>
                           ))}
                        </div>
                        <h3 className="text-2xl font-medium text-slate-900 mb-1" style={{ fontFamily: "serif" }}>{project.title}</h3>
                        <div className="flex items-center gap-1.5 text-slate-500 text-sm font-medium mb-4">
                           <MapPin className="w-4 h-4" /> {project.subtitle}
                        </div>
                        <p className="text-sm text-slate-600 leading-relaxed mb-6 flex-1">
                           {project.desc}
                        </p>
                        <a href="#contact" className="inline-flex items-center gap-2 text-sm font-semibold text-[#8B4A46] hover:text-slate-900 transition-colors group/link mt-auto">
                           Use this Template <ArrowRight className="w-4 h-4 group-hover/link:translate-x-1 transition-transform" />
                        </a>
                     </div>
                  </motion.div>
               ))}
               ...
               >
                  ...
                  <ul className="space-y-4 mb-8 flex-1">
                     {[
                        ...
                        { name: 'Fast 24-hour delivery', desc: 'Get your online presence up and running quickly', icon: <Zap className="w-5 h-5 text-[#8B4A46] shrink-0 mt-0.5" /> }
                     ].map((feature, i) => (
